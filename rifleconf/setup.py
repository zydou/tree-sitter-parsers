...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_rifleconf", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_rifleconf": ["*.pyi", "py.typed"],
        "tree_sitter_rifleconf.queries": ["*.scm"],
    },
    ext_package="tree_sitter_rifleconf",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_rifleconf/binding.c",
                "src/parser.c",
            ],
            define_macros=[
